...
#define PCII_RREG8_(bus, dev, func, reg) \
	(outl(PCII_SELREG_(bus, dev, func, reg), PCII_CONFADD), \
	inb(PCII_CONFDATA+((reg)&3)))
#define PCII_RREG16_(bus, dev, func, reg) \
	(outl(PCII_SELREG_(bus, dev, func, reg), PCII_CONFADD), \
	inw(PCII_CONFDATA+((reg)&2)))
...
#define PCII_WREG16_(bus, dev, func, reg, val) \
	(outl(PCII_SELREG_(bus, dev, func, reg), PCII_CONFADD), \
	outw((val), PCII_CONFDATA+((reg)&2)))
#define PCII_WREG32_(bus, dev, func, reg, val) \
	(outl(PCII_SELREG_(bus, dev, func, reg), PCII_CONFADD), \
	outl((val), PCII_CONFDATA))
...
#define SUBCLASS_NET  0x02   /* subclass 0x2 is refering to network */
#define SUBCLASS_BRDG 0x06
#endif /* __PCI_H__ */
